...
def keep_call_info_synced(but, bot_token, calls_chat_id):
    ...
    while True:
        ...
        calls_with_files, amount_calls = get_calls_with_files(but, first_call_id)  # использовать get_calls_with_files
        if calls_with_files:
            bot.send_message(chat_id=calls_chat_id, text='Есть не загруженные звонки')
...
def export_calls_to_telegram(but, bot_token, calls_chat_id):
    bot = Bot(token=bot_token)
    if but.call_api_method('app.option.get')['result']['last_call_id']:
        first_call_id = but.call_api_method('app.option.get')['result']['last_call_id']
    else:
        first_call_id = "0"
