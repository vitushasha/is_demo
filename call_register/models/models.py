...
class NumberChoicesVOTE(models.IntegerChoices):
    one = 1
    two = 2
    three = 3
    four = 4
...
class CustomCallInfo(models.Model):
    ...
    vote = models.IntegerField(choices=NumberChoicesVOTE.choices, null=True, blank=True)
    ...
    def telephony_externalcall_register(self, but):
        ...
        self.filename = str(self.file)[len(self.inner_media_path):-len(os.path.splitext(str(self.file))[-1])] + '.mp3'
    ...
    """def wav_maker_n_messages(self, but):
        ne_path = os.path.join(settings.BASE_DIR, "media", self.inner_media_path, self.filename).replace("\\", "/")
